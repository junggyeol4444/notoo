...
def strip_particle(
    ...
    "않았다" 같은 서술어가 전부 명사 후보로 들어온다. 서술어는 조사가
    붙지 않으므로 이 규칙만으로 걸러진다.

    서술어 어미 검사는 어절이 아니라 '조사를 뗀 어간'에만 건다. 어절에
    걸면 조사 자체가 서술어 어미로 끝나는 경우에 막힌다. 실제로
    에게 / 에서 / 께서 / 라고 / 까지 / 보다 / 에게서 / 한테서 / 이라고가
    각각 게·서·고·지·다로 끝나서 전부 차단됐었다.
    """
