...
POSITIVE_WORDS: set[str] = {
    "기쁘", "기뻐", "행복", "웃었", "웃음", "웃으", "미소", "환호", "안도", "따뜻", "다행", "성공",
    "승리", "이겼", "해냈", "완벽", "만족", "뿌듯", "감동", "사랑", "설레", "기대",
    "희망", "빛나", "개운", "시원", "여유", "평온", "든든", "믿음", "고맙", "감사",
    "칭찬", "인정", "보상", "획득", "각성", "성장", "돌파", "승진", "계약", "체결",
    "합격", "부활", "회복", "구했", "살렸", "재회", "화해", "축하", "환하", "확신",
}

NEGATIVE_WORDS: set[str] = {
    "슬프", "슬퍼", "눈물", "울었", "울음", "흐느", "분노", "화났", "짜증", "불안",
    "두려", "무서", "공포", "절망", "좌절", "패배", "졌다", "실패", "무너", "부서",
    "죽었", "죽음", "죽어", "죽일", "시체", "핏자국", "피를", "피가", "출혈",
    "비명", "고통", "아프", "상처", "배신", "속였", "거짓", "위기", "위험",
    "경고", "협박", "위협", "적대", "원수", "증오", "미워", "혐오", "역겨", "구역",
    "외로", "고독", "쓸쓸", "허무", "막막", "답답", "초조", "긴장", "식은땀",
    "떨렸", "싸늘", "차갑", "굳었", "얼어", "끔찍", "참담", "비참", "억울", "원망",
    "포기", "상실", "잃었", "무력", "파산", "폭락", "해고", "부도", "소송", "고소",
}
...
EVENT_WEIGHTS: dict[str, float] = {
    # 전투/충돌
    "전투": 2.0, "싸움": 1.5, "격돌": 2.0, "공격": 1.5, "베었": 1.8, "찔렀": 1.8,
    "쓰러": 1.5, "폭발": 2.5, "무너졌": 2.0, "총성": 2.2, "칼날": 1.5,
    # 생사
    "죽었": 3.0, "죽음": 2.8, "사망": 3.0, "시체": 2.5, "장례": 2.2, "살해": 3.0,
    "부활": 2.8, "회귀": 3.0, "각성": 2.5,
    # 관계 전환
    "배신": 2.8, "고백": 2.0, "이별": 2.0, "재회": 1.8, "약속": 1.2, "맹세": 1.8,
    "결혼": 2.2, "이혼": 2.2,
    # 정보/반전
    "진실": 2.0, "정체": 2.2, "밝혀졌": 2.2, "알고 있었": 2.0, "비밀": 1.8,
    "함정": 2.2, "음모": 2.0, "증거": 1.8, "폭로": 2.5,
    # 경영/현대물
    "인수": 2.2, "합병": 2.2, "계약": 1.5, "체결": 1.5, "투자": 1.5, "상장": 2.2,
    "부도": 2.5, "파산": 2.5, "소송": 2.0, "고소": 2.0, "해고": 1.8, "폭락": 2.2,
    "급등": 1.8, "인사": 1.0,
    # 지위 변화
    "승진": 1.8, "취임": 2.0, "몰락": 2.5, "추방": 2.2, "탈출": 2.0, "체포": 2.2,
    "구속": 2.2, "석방": 2.0,
    # 등장/퇴장
    "나타났": 1.5, "등장": 1.3, "사라졌": 1.8, "떠났": 1.5,
}
...
CLIFFHANGER_PATTERNS: dict[str, list[str]] = {
    "정보공개": [
        "사실은", "진실", "정체는", "알고 보니", "밝혀졌", "이었다니", "였다니",
        "그 사람이", "네가", "당신이", "바로 그", "폭로",
    ],
    "위기발생": [
        "위험", "위기", "경보", "비명", "쓰러졌", "피를", "무너", "터졌", "불길",
        "늦었", "당했", "덮쳤", "포위", "갇혔", "도망",
    ],
    "새로운적": [
        "처음 보는", "낯선", "누구", "정체불명", "그림자", "나타났", "등장했",
        "가로막", "막아섰", "노려보",
    ],
    "반전": [
        "하지만", "그러나", "설마", "말도 안", "믿을 수 없", "예상", "뒤집",
        "반대로", "착각", "속았", "함정",
    ],
    "약속": [
        "반드시", "기다려", "다음에", "약속", "맹세", "돌아오", "끝내겠", "갚아",
        "두고 보",
    ],
    "미스터리": [
...
ACTION_WORDS: set[str] = {
    "걸었", "걸어", "뛰었", "달렸", "움직였", "일어섰", "앉았", "누웠", "돌아섰",
    "멈췄", "다가갔", "물러섰", "뻗었", "잡았", "놓았", "던졌", "밀었", "당겼",
    "열었", "닫았", "들었", "내려놓", "건넸", "받았", "꺼냈", "집었", "휘둘",
    "내리쳤", "후려쳤", "때렸", "걷어찼", "피했", "막았", "올랐", "내렸",
    "들어섰", "나섰", "향했",
    "고개를", "손을", "발을", "몸을", "눈을", "입을", "어깨를", "시선을",
}

PSYCH_WORDS: set[str] = {
    "생각했", "생각이", "느꼈", "느낌", "깨달았", "떠올", "기억", "판단", "짐작",
    "확신", "의심", "궁금", "바랐", "원했", "싶었", "두려", "불안", "안도", "후회",
    "결심", "각오", "마음", "속으로", "머릿속", "가슴이", "심장이", "머리가",
    "이해", "납득", "혼란", "복잡", "직감", "예감", "본능",
}
...
NAME_PARTICLES = (
    "은", "는", "이", "가", "을", "를", "에게", "한테", "의", "와", "과", "도",
    "만", "아", "야", "씨", "님", "이는", "이가", "이를", "이에게", "이도",
)

# 인물 후보에서 걸러내야 하는 흔한 명사/부사. 2~4음절 한글 덩어리를 뽑으면
# 아래 단어들이 이름처럼 잡힌다.
NAME_STOPWORDS: set[str] = {
    "그것", "이것", "저것", "여기", "거기", "저기", "사람", "사람들", "남자",
    "여자", "아이", "어른", "모두", "우리", "너희", "자신", "자기", "당신",
    "누구", "무엇", "어디", "언제", "지금", "오늘", "내일", "어제", "방금",
    "순간", "잠시", "동안", "사이", "안에", "밖에", "위에", "아래", "앞에",
    "뒤에", "옆에", "속에", "다시", "아직", "이미", "벌써", "정말", "진짜",
    "조금", "많이", "너무", "가장", "제일", "그냥", "역시", "결국", "물론",
    "하지만", "그러나", "그리고", "그래서", "그러자", "그때", "이번", "다음",
    "마지막", "처음", "얼굴", "목소리", "시선", "고개", "머리", "가슴", "마음",
    "생각", "기분", "표정", "눈빛", "손끝", "입가", "회사", "사무실", "건물",
